...
namespace LoreCompanion.Views.Controls
{
    ...
    public class SeparatingPanel : Panel
    {
        ...
        {
            _ = WrapMeasuredChildren()
                .Aggregate(
                    0.0,
                    (top, wrapper) =>
                    {
                        var finalRect = new Rect(
                            new Point(0.0, top + wrapper.SeparatorHeight),
                    ...
                    (top, wrapper) =>
                    {
                        if (wrapper.HasSeparator)
                        {
                            ...
                            var bottomRight = new Point(
                                ActualWidth - SeparatorMargin.Right,
        ...
        private ChildWrapper[] WrapMeasuredChildren()
        {
            if (InternalChildren.Count == 0)
            {
            ...
            Array.ForEach(
                wrappers,
                wrapper => { wrapper.TotalHeight = wrapper.ChildHeight + wrapper.SeparatorHeight; });
